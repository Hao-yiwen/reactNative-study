...
const SecondLevelComponent = ({setFirstLevelState}) => {
  ...
  useEffect(() => {
    ...
    return () => {
      console.log(performance.now() + ' SecondLevelComponent remove render time');
    };
  });
...
const ThirdLevelComponent = ({setSecondLevelState}) => {
  ...
  useEffect(() => {
    ...
    return () => {
      console.log(performance.now() + ' ThirdLevelComponent remove render time');
    };
  });

  return (
    <View style={{padding: 20, backgroundColor: '#b0b0b0', marginBottom: 10}} ref={viewRef}>
